...
function App() {
  ...
  async function updateCloudProject(id: string | null) {
    if (!user) { notifyError('Sign in to update projects'); return; }
    if (!id) { notifyError('No project selected'); return; }
    if (!plot) { notifyError('No plot to save'); return; }
    setNetworkLoading(true);
    try {
      const d = doc(db, 'users', user.uid, 'projects', id);
      ...
      notifySuccess('Updated cloud project');
      await fetchCloudProjects();
    } catch (e) {
      console.error(e);
      notifyError('Failed updating project');
    } finally {
  ...
  function handleExportBlender() {
    ...
    try {
      ...
      notifySuccess('Blender script downloaded');
    } catch (e) {
  ...
  const height = useGlobalStore((s) => (s as any).height);
    ...
    function handleMenuBarAction(e: Event) {
      ...
      switch (action) {
        ...
        case 'Flip Horizontal':
          ...
          notifySuccess('Flipped horizontally');
          break;
        case 'Flip Vertical':
          setPlot(flipVertical(plot));
          notifySuccess('Flipped vertically');
          break;
        case 'Rotate Left':
          setPlot(rotateLeft(plot));
          notifySuccess('Rotated left');
          break;
        case 'Rotate Right':
          setPlot(rotateRight(plot));
          notifySuccess('Rotated right');
          break;
        default:
          break;
      }
    ...
    navigator.clipboard.readText().then(txt => {
      try {
        ...
        if (Array.isArray(arr) && Array.isArray(arr[0])) {
          setPlot(arr);
          notifySuccess('Pasted from clipboard');
        }
      } catch { notifyError('Clipboard data invalid'); }
    });
  }
  ...
  async function fetchCloudProjects() {
    ...
    try {
      ...
      console.error(e);
      notifyError('Failed fetching cloud projects');
    } finally {
      setNetworkLoading(false);
    }
  }
  ...
  async function loadCloudProject(p: any) {
    ...
    if (p.plot) {
      ...
      notifySuccess('Loaded project from Firestore');
    }
  }
  ...
  async function performDeleteCloudProject() {
    ...
    if (!id) return;
    if (!user) { notifyError('Sign in to delete cloud projects'); setDeleteConfirmOpen(false); return; }
    setNetworkLoading(true);
    try {
      ...
      notifySuccess('Deleted cloud project');
    } catch (e) {
      console.error(e);
      notifyError('Failed deleting project');
    } finally {
      setNetworkLoading(false);
  ...
  function applySelectionAsCurrent(id: string | null) {
    ...
    notifyInfo('Selected cloud project set for updates');
  }
  useEffect(() => {
  ...
  useEffect(() => {
    function calculateCellSize() {
      ...
      const cols = plot ? (plot[0]?.length ?? (width || 16)) : (width || 16);
      const candidate = Math.floor(containerWidth / cols);
  ...
  function handleClear() {
    // Clear the current board using the current width x height
    const dsMap: string[][] = Array.from({ length: (height as number) }, () => Array.from({ length: (width as number) }, () => "transparent"));
    setPlot(dsMap);
    notifySuccess("Board cleared");
  }
  ...
  function saveProject(mode: 'save' | 'saveAs' = 'save') {
    if (!plot) {
      notifyError('No plot to save')
      return
    }
    try {
      const data = { plot, voxelSizeMm, title: title || 'Untitled', width, height }
  ...
  function exportPNG() {
    ...
    cvs.toBlob((blob) => {
      if (!blob) { notifyError('Export failed'); return }
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
  ...
  useEffect(() => {
    try {
      ...
      if (s) {
        ...
        // Backwards compatibility for legacy `size` value
        if (typeof parsed.size === 'number') { setWidth(parsed.size); setHeight(parsed.size); }
  ...
  return (
    <div className="App">
      ...
      {/* Notifications removed — using console-based shims. */}
      <header>
