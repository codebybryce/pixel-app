...
export function generateBlenderScript(plot: string[][], voxelSizeMm: number): string | null {
  ...
  // Convert mm to Blender units (meters): 1 mm = 0.001 m
  const layerThickness = Number((voxelSizeMm / 1000).toFixed(6));
...
SCALE = 10  # scale model by this factor
...
for i, row in enumerate(plot):
    for j, color in enumerate(row):
        ...
        # Each color occupies one LAYER_THICKNESS slice stacked upward
        z_center = idx * LAYER_THICKNESS + (LAYER_THICKNESS / 2.0)

        xpos = i * LAYER_THICKNESS - x_offset + (LAYER_THICKNESS / 2.0)
        ypos = -(j * LAYER_THICKNESS - y_offset + (LAYER_THICKNESS / 2.0))

        # Create a cube of size LAYER_THICKNESS positioned at the appropriate Z slice
...
if created:
    ...
    # Scale the unioned mesh (before applying transforms) to achieve requested model scale
    union_obj.scale = (SCALE, SCALE, SCALE)
    ...
    mod.mode = 'VOXEL'
    # Adjust voxel size for remesh proportional to model scale
    mod.voxel_size = (LAYER_THICKNESS * SCALE) / 20  # half a voxel for clean union
    bpy.ops.object.modifier_apply(modifier=mod.name)

    # Recenter to origin (Z base already at 0)
    union_obj.location.z = 0.4 * SCALE
`;
